...
$$
	...
	\begin{cases}
		1,& \exists t \in (\Fqd)^\times : t^2  = x[q^{d_i}] \\
		-1,& \not\exists t \in (\Fqd)^\times : t^2  = x[q^{d_i}] \\
		0, & x = 0.
	\end{cases}
$$
Покажем, что вычисление $\left( \dfrac{\cdot}{\Fqd} \right)$ сводится к вычислению $\left( \dfrac{\cdot}{\F_q} \right)$. \\
В наших примерах $q$ — простое $\Rightarrow \left( \dfrac{\cdot}{\F_q} \right) = \left( \dfrac{\cdot}{q} \right) $ — символ Лежандра $\Rightarrow \exists$ эффективный алгоритм, т. ч. $\left(\dfrac{x}{q}\right) = x^{\frac{q-1}{2}}[q]$.
